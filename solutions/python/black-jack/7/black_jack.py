"""Functions to help play and score a game of blackjack.

How to play blackjack:    https://bicyclecards.com/how-to-play/blackjack/
"Standard" playing cards: https://en.wikipedia.org/wiki/Standard_52-card_deck
"""
# --- Type Aliases ---
from typing import TypeAlias

Card: TypeAlias = str
HigherCardResult: TypeAlias = Card | tuple[Card, Card]


def value_of_card(card: Card) -> int:
    """Determine the scoring value of a card.

    :param card: Card - given card.
    :return: int - value of a given card.  See below for values.

    1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
    2.  'A' (ace card) = 1
    3.  '2' - '10' = numerical value.
    """
    if card == "A":
        return 1
    if card in {"J", "Q", "K"}:
        return 10
    return int(card)


def higher_card(card_one: Card, card_two: Card) -> HigherCardResult:
    """Determine which card has a higher value in the hand.

    :param card_one, card_two: Card - cards dealt in hand.  See below for values.
    :return: Card or tuple[Card, Card] - resulting Tuple contains both cards if they are of equal value.

    1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
    2.  'A' (ace card) = 1
    3.  '2' - '10' = numerical value.
    """
    value_one = value_of_card(card_one)
    value_two = value_of_card(card_two)

    if value_one > value_two:
        return card_one
    if value_two > value_one:
        return card_two
    return card_one, card_two


def value_of_ace(card_one: Card, card_two: Card) -> int:
    """Calculate the most advantageous value for the ace card.

    :param card_one, card_two: Card - card dealt. See below for values.
    :return: int - either 1 or 11 value of the upcoming ace card.

    1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
    2.  'A' (ace card) = 11 (if already in hand)
    3.  '2' - '10' = numerical value.
    """
    hand = (card_one, card_two)
    if "A" in hand:
        return 1
    total = sum(value_of_card(card) for card in hand)
    return 11 if total <= 10 else 1


def is_blackjack(card_one: Card, card_two: Card) -> bool:
    """Determine if the hand is a 'natural' or 'blackjack'.

    :param card_one, card_two: Card - card dealt. See below for values.
    :return: bool - is the hand is a blackjack (two cards worth 21).

    1.  'J', 'Q', or 'K' (otherwise known as "face cards") = 10
    2.  'A' (ace card) = 11 (if already in hand)
    3.  '2' - '10' = numerical value.
    """
    hand = (card_one, card_two)
    return "A" in hand and any(value_of_card(card) == 10 for card in hand)


def can_split_pairs(card_one: Card, card_two: Card) -> bool:
    """Determine if a player can split their hand into two hands.

    :param card_one, card_two: Card - cards dealt.
    :return: bool - can the hand be split into two pairs? (i.e. cards are of the same value).
    """
    return value_of_card(card_one) == value_of_card(card_two)


def can_double_down(card_one: Card, card_two: Card) -> bool:
    """Determine if a blackjack player can place a double down bet.

    :param card_one, card_two: Card - first and second cards in hand.
    :return: bool - can the hand can be doubled down? (i.e. totals 9, 10 or 11 points).
    """
    hand = (card_one, card_two)
    total_value = sum(value_of_card(card) for card in hand)
    return total_value in {9, 10, 11}
